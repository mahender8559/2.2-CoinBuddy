...
text = replace_once(
    ...
    "const LIQUID_ACCOUNT_GROUPS = new Set([\n  'bank',\n  'bank account',",
    'legacy bank subtype as liquid cash',
)
...
text = replace_once(
    ...
    'projection accumulator liability coverage',
)

helper_marker = "function applyProjectedTransaction(\n"
helpers = r'''function recordLiabilityPayment(
...
if text.count(helper_marker) != 1:
    raise RuntimeError(f'liability helper marker: expected exactly one match, found {text.count(helper_marker)}')
text = text.replace(helper_marker, helpers + helper_marker, 1)

old_expense = r'''  if (type === 'EXPENSE') {
    if (fromLiquid) {
...
'''
text = replace_once(text, old_expense, new_expense, 'expense liability payment handling')

old_transfer = r'''  const liquidDelta = (toLiquid ? amount : 0) - (fromLiquid ? amount : 0);
  if (liquidDelta > 0) {
  ...
  accumulator.occurrenceCount += 1;
'''
new_transfer = r'''  const liquidDelta = (toLiquid ? amount : 0) - (fromLiquid ? amount : 0);
  if (liquidDelta > 0) {
    accumulator.otherCashInflows += liquidDelta;
  } else if (liquidDelta < 0) {
    const outflow = Math.abs(liquidDelta);
  ...
  accumulator.occurrenceCount += 1;
'''
text = replace_once(text, old_transfer, new_transfer, 'transfer liability payment handling')

old_loan = r'''function projectLoanFallbacks(
  accounts: Account[],
  coveredLiabilityIds: Set<string>,
...
'''
new_loan = r'''function projectLoanFallbacks(
  accounts: Account[],
  creditCardIds: Set<string>,
  asOfDate: string,
  endDate: string,
...
'''
text = replace_once(text, old_loan, new_loan, 'loan fallback de-duplication')

text = replace_once(
    text,
    "    coveredLiabilityIds: new Set<string>(),\n    occurrenceCount: 0,",
    "    liabilityPaymentsByDate: new Map<string, Map<string, number>>(),\n    occurrenceCount: 0,",
    'projection accumulator initialization',
)

old_cards = r'''  const creditCards = input.creditCards ?? [];
...
'''
new_cards = r'''  // A card record is only authoritative while its backing liability account
...
'''
text = replace_once(text, old_cards, new_cards, 'credit card and loan fallback integration')

path.write_text(text, encoding='utf-8')

phase3_tests = r'''import { describe, expect, it } from 'vitest';
...
'''
Path('src/domain/affordability.phase3.test.ts').write_text(phase3_tests, encoding='utf-8')

print('Affordability Phase 3 hardening applied.')
