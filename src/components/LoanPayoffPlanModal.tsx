...
export function LoanPayoffPlanModal({ account, onClose }: { account: Account | null; onClose: () => void }) {
  ...
  return <V35ModalFrame size="lg" testId="loan-payoff-plan-sheet" labelledBy="loan-payoff-plan-title" panelClassName="p-0">
    ...
    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4 sm:p-5">
      ...
      {plan && summary ? <>
        <section className="rounded-2xl border border-primary/20 bg-primary/5 p-4"><div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-primary"><LockKeyhole className="h-4 w-4" /><span className="text-xs font-bold uppercase tracking-wide">Payoff progress</span></div><strong className="mt-2 block text-2xl font-numeric text-on-surface">{formatCurrency(summary.fundedAmount)}</strong><p className="mt-1 text-xs text-on-surface-variant">of {formatCurrency(summary.target)} · {summary.progress.toFixed(0)}% funded</p><p className="mt-1 text-[10px] text-on-surface-variant">Ready now {formatCurrency(summary.reserved)} · already paid {formatCurrency(summary.consumed)}</p></div>{summary.funded ? <span className="flex items-center gap-1 rounded-full bg-emerald-500/10 px-2.5 py-1 text-[10px] font-semibold text-emerald-400"><CheckCircle2 className="h-3.5 w-3.5" /> Funded</span> : null}</div><div className="mt-3 h-2 overflow-hidden rounded-full bg-surface-container-high"><div className="h-full rounded-full bg-primary" style={{ width: `${summary.progress}%` }} /></div><p className="mt-2 text-xs text-on-surface-variant">{formatCurrency(summary.remaining)} still needs to be reserved.</p></section>
