...
import { X, Utensils, Car, Briefcase, Zap, Home, ShoppingBag, Banknote, Plus, ShieldCheck, Layers, ChevronUp, ChevronDown, Calendar as CalendarIcon, Edit3, Lock, CreditCard, Landmark, Check, AlertTriangle } from 'lucide-react';
...
export function AddTransactionModal() {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!amount || isNaN(numAmount) || numAmount <= 0) {
      setError('Transaction amount must strictly be a positive number (> 0).');
      return;
    }
    ...
    if (type === 'income') {
      ...
      if (selectedAcc && selectedAcc.type === 'liability') {
        setError('Credit Cards and Loans (Liabilities) cannot be selected as the destination account for Income. Income can only flow into Asset accounts.');
        return;
      }
    }
    ...
    if (type === 'expense') {
      ...
      if (selectedAcc && selectedAcc.type === 'asset') {
        ...
        if (numAmount > availableBalance) {
          setError(`Insufficient funds in ${selectedAcc.name}. Cannot process transaction.`);
          return;
        }
      }
      ...
      if (sourceAcc && sourceAcc.type === 'asset') {
        ...
        if (numAmount > availableBalance) {
          setError(`Insufficient funds in ${sourceAcc.name}. Cannot process transaction.`);
          return;
        }
      }
    }
    ...
    const newTx = {
      ...
      category: type === 'transfer' ? '#transfer' : (type === 'income' ? '#income' : `#${categoryName.toLowerCase().replace(/\s+/g, '')}`),
      icon: type === 'transfer' ? 'ArrowRightLeft' : (type === 'income' ? 'ArrowDownToLine' : iconName),
      type,
    ...
    if (!res.success) {
      setError(res.error || 'Insufficient funds. Cannot process transaction.');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-6 max-w-2xl mx-auto">
          ...
          <div className="flex flex-col items-center justify-center py-6">
            ...
            <div className="flex items-center justify-center gap-3">
              <span className="text-4xl md:text-5xl font-bold text-primary">{getCurrencySymbol()}</span>
              <input 
                type="number"
                step="0.01"
                required
                ...
                className="bg-transparent text-5xl md:text-6xl font-numeric font-bold text-on-surface focus:outline-none w-48 text-center placeholder:text-on-surface-variant/30"
                placeholder="0.00"
              />
          ...
          {type === 'expense' && (
            <div className="bg-surface-container-low rounded-2xl border border-outline-variant/30 p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider">CATEGORY</h3>
                <span 
          ...
          {type !== 'transfer' && (
            <div className="bg-surface-container-low rounded-2xl border border-outline-variant/30 p-5">
              <h3 className="text-xs font-semibold text-on-surface-variant uppercase tracking-wider mb-4">ACCOUNT</h3>
              <div className="flex gap-3 flex-wrap">
                {(type === 'income' ? assets : accounts).map(acc => (
                  <button
                    key={acc.id}
                    type="button"
          ...
          <div className="pt-4 space-y-4">
            <button 
              type="submit"
              className="w-full bg-primary hover:bg-primary/90 text-on-primary font-bold py-4 rounded-2xl transition-colors flex items-center justify-center gap-2 text-lg"
            >
              <Lock className="w-5 h-5" /> Save Transaction
            </button>
            <p className="text-center text-xs text-on-surface-variant">Saved instantly to your secure local ledger.</p>
          </div>
        </form>
      </div>
    </div>
