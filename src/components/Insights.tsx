...
export function Insights() {
  ...
  const assets = useMemo(() => accounts.filter(a => a.type === 'asset' && a.balance > 0).sort((a, b) => b.balance - a.balance), [accounts]);
  const totalAssets = assets.reduce((sum, a) => sum + a.balance, 0);
  
  const liabilities = useMemo(() => accounts.filter(a => a.type === 'liability').sort((a, b) => b.balance - a.balance), [accounts]);
  ...
  const categoryTotals = useMemo(() => {
    ...
    transactions.filter(t => {
      if (t.isOpeningBalance || t.type !== 'expense' || !isDateInCurrentCycle(t.date)) return false;
  ...
  const availableCategories = useMemo(() => {
    ...
    transactions.filter(t => !t.isOpeningBalance && t.type === 'expense').forEach(t => set.add(t.category));
  ...
  const categoryTrendData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      transactions.forEach(t => {
        if (t.isOpeningBalance || t.type !== 'expense') return;
    ...
    const catTxs = transactions.filter(t => 
      !t.isOpeningBalance && t.type === 'expense' && (selectedCategory === 'all' || t.category === selectedCategory)
    );
  ...
  const smartTips = useMemo(() => {
    ...
    const recurringTxs = transactions.filter(t => !t.isOpeningBalance && t.type === 'expense' && (t.title.toLowerCase().includes('subscription') || t.title.toLowerCase().includes('netflix') || t.title.toLowerCase().includes('spotify')));
  ...
  const liabilityMetrics = useMemo(() => {
    return liabilities.map(liability => {
      ...
      } else if (liability.originalPrincipal) {
        totalCapacity = liability.originalPrincipal;
        paidOff = Math.max(0, totalCapacity - liability.balance);
        percentPaid = totalCapacity > 0 ? (paidOff / totalCapacity) * 100 : 0;
      } else {
        percentPaid = liability.balance === 0 ? 100 : 50;
      }
