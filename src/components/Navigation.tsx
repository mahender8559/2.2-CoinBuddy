...
export function Navigation({ activeTab, setActiveTab }: NavProps) {
  ...
  return (
    <>
      {/* Mobile Bottom Nav */}
      <nav className="md:hidden fixed bottom-0 w-full z-50 bg-surface-container-low border-t border-outline-variant/50 px-1 py-1 pb-safe flex justify-around items-center">
        {tabs.map(({ id, label, icon: Icon }) => (
          <button
            key={id}
            onClick={() => setActiveTab(id as Tab)}
            className={`flex flex-col items-center p-1.5 rounded-xl min-w-0 flex-1 transition-colors ${
              activeTab === id 
          ...
          >
            ...
            <span className="text-[9px] xs:text-[10px] font-semibold leading-tight">{label}</span>
          </button>
        ))}
      </nav>
