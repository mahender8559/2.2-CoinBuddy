...
export function ManageFinances() {
  const { categories, addCategory, updateCategory, deleteCategory, formatCurrency, transactions, getCurrencySymbol, isDateInCurrentCycle } = useAppContext();
  
  const [mainTab, setMainTab] = useState<'Accounts' | 'Categories'>('Accounts');
  ...
  const hasPriorData = transactions.filter(t => !isDateInCurrentCycle(t.date) && !t.isOpeningBalance).length > 0;
  ...
  const getSavingsTotal = (catId: string, catName: string) => {
    const catTag = `#${catName.toLowerCase().replace(/\s+/g, '')}`;
    return transactions
      .filter(t => !t.isOpeningBalance && t.type === 'expense' && (t.category === catTag || t.category === catId))
  ...
  const saveCategory = () => {
    ...
    if (editingId) {
      updateCategory(editingId, { name: editName, icon: editIcon, budget: editBudget });
    } else {
      addCategory({ name: editName, icon: editIcon, budget: editBudget, group: activeTab === 'Savings Goals' ? 'Savings' : 'Essential' });
    }
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
          {['All', 'Essential', 'Leisure'].map(group => (
            <button
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap transition-colors ${filterGroup === group ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-on-surface hover:bg-surface-variant'}`}
            >
              {group}
            </button>
          ))}
            ...
            <div className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  placeholder={activeTab === 'Savings Goals' ? 'e.g. Vacation Fund' : 'e.g. Groceries'}
                />
              </div>
